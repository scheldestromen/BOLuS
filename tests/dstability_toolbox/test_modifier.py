...
class TestModifierAddSoilCollection(TestCase):
    def setUp(self):
        self.soil_collection = SoilCollection(soils=[
...
class TestModifierSetSubsoil(TestCase):
    def setUp(self):
        ...
        self.subsoil = Subsoil(soil_polygons=[
            SoilPolygon(soil_type='Klei', points=[(0, 0), (0, 1), (1, 1), (1, 0)]),
            SoilPolygon(soil_type='Zand', points=[(0, 0), (0, -1), (1, -1), (1, 0)])
        ])
    ...
    def test_set_subsoil(self):
        ...
        dm = set_subsoil(self.subsoil, dm, scenario_index=scenario_index, stage_index=stage_index)

        # Run test
        self.assertEqual(len(dm.datastructure.soillayers[0].SoilLayers),  # There is only 1 soillayers
...
class TestModifierAddUniformLoad(TestCase):
    def setUp(self):
        ...
        self.char_points_profile_outward_positive = CharPointsProfile(name='test', points=char_points_outward_positive)
        self.char_points_profile_inward_positive = CharPointsProfile(name='test', points=char_points_inward_positive)

        self.soil_collection = SoilCollection(soils=[
            Soil(gl_soil=GLSoil(name='Klei', code='Klei'), consolidation_traffic_load=50),
            Soil(gl_soil=GLSoil(name='Zand', code='Zand'), consolidation_traffic_load=100)
        ])
        self.subsoil = Subsoil(soil_polygons=[
            SoilPolygon(soil_type='Klei', points=[(0, 0), (10, 4), (20, 4), (30, 0)]),
            SoilPolygon(soil_type='Zand', points=[(0, -5), (0, 0), (30, 0), (30, -5)])
        ])

        dm = DStabilityModel()
        dm = add_soil_collection(self.soil_collection, dm)
        self.dm = set_subsoil(subsoil=self.subsoil, dm=dm, scenario_index=0, stage_index=0)
    ...
    def test_outward_positive_load_direction_outward(self):
        ...
        # Run test
        self.assertEqual(self.dm.datastructure.loads[0].UniformLoads[0].Magnitude, load.magnitude)
        self.assertEqual(self.dm.datastructure.loads[0].UniformLoads[0].Spread, load.angle)
        ...
        consolidations = [cons.Degree for cons in self.dm.datastructure.loads[0].UniformLoads[0].Consolidations]
...
class TestSetWaternet(TestCase):
    ...
    def test_set_waternet(self):
        dm = DStabilityModel()
        dm = set_waternet(waternet=self.waternet, dm=dm, scenario_index=0, stage_index=0)
...
class TestCreateDStabilityModel(TestCase):
    ...
    def test_create_d_stability_model(self):
        ...
        self.assertEqual(len(dm.scenarios[0].Stages), len(self.model.scenarios[0].stages))
        self.assertEqual(len(dm.soils.Soils), len(self.model.soil_collection.soils))
        self.assertEqual(len(dm.datastructure.states[0].StatePoints),
