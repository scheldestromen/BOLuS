...
def get_by_id(collection: list[Any], item_id: str) -> Any:
    ...
    item = next(
        (item for item in collection if item_id == getattr(item, 'Id', None)),
        None
    )
...
def get_calculation_settings_by_result_id(dm: DStabilityModel, result_id: str) -> CalculationSettings:
    ...
    return get_by_id(collection=dm.datastructure.calculationsettings, item_id=calculation.CalculationSettingsId)
